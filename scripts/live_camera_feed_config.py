...
"""
Live camera feed viewer using predefined camera configuration.
...
```shell
python scripts/live_camera_feed_config.py
```
"""
    ...
    try:
        while True:
            ...
            for camera_name, cam_dict in cameras.items():
                ...
                try:
                    # Read image data
                    image_data = cam_dict["instance"].async_read()
                    torch_data = torch.from_numpy(image_data).type(torch.float32) / 255
                    torch_data = torch_data.permute(2, 0, 1).contiguous()
                    import torchvision.transforms.functional as F  # type: ignore  # noqa: N812

                    torch_data = F.resize(torch_data, (480, 640))

                    # invert the above operations
                    torch_data = torch_data.permute(1, 2, 0).contiguous()
                    torch_data = torch_data * 255
                    torch_data = torch.clip(torch_data, 0, 255)
                    image_data = torch_data.to(torch.uint8).numpy()
